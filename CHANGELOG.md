...
- Les analyses (superman, performance, régularité, regroupement, lièvre/suiveur, récapitulatif, duel) sont **bloquées pendant la course** : tant qu'un coureur parti (heure de départ passée) n'a pas de statut attribué par MeOS (lecture de puce à la GEC ou statut définitif), aucune analyse n'est calculée — message dédié avec lien vers le live, déblocage automatique à la fin de la course. Les coureurs jamais partis (`st = 0`) ou au départ futur ne bloquent pas ; l'export CSV du récapitulatif répond 409 pendant la course.
